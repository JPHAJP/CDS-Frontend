...
## Demo Users For Automated Tests

Use these disposable demo accounts for TestSprite scenarios that require non-admin users:

- Authorized volunteer:
  - Email: `demo.voluntario@test.local`
  - Password: `DemoPassword123!`
  - Role: `voluntarios`
  - State: `authorized`
  - Intended tests: user dashboard, profile, QR scan access, authorized-user routing.
- Pending worker:
  - Email: `demo.personal@test.local`
  - Password: `DemoPassword123!`
  - Role: `personal`
  - State: `pending`
  - Intended tests: admin pending list, role filter, identification preview, authorize/reject workflows.

Both demo users have a PNG identification file available through the admin identification endpoint. Destructive admin tests may mutate these users; reseed them before repeating the same workflow tests.
